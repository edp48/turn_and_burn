...
load totalrun_min.mat opt_initial_vec

old_init_v1 = opt_initial_vec(:,1:33);
old_init_v2 = opt_initial_vec(:,34:end);
clear opt_initial_vec
...
[opt_initial_vec1(:,1),fval,exitflag,output]= fminsearch(@(guess_init) cost_function_matlab(guess_init,p,funs),guess_init,options);

[TOF_v1(1),m_fuel_v1(1),dV_v1(1),flip_t_v1(1),e_s_v1(1),leadvtrail1(1)] = extract_info(opt_initial_vec1(:,1),p,funs);
...
j
options = optimset('MaxFunEval',5000,'MaxIter',70,'Display','iter','TolFun',1e-8,'TolX',1e-8);

[opt_initial_vec1(:,j),fval,exitflag,output]= fminsearch(@(guess_init) cost_function_matlab(guess_init,p,funs),old_init_v1(:,j),options);



[TOF_v1(j),m_fuel_v1(j),dV_v1(j),flip_t_v1(j),e_s_v1(j),leadvtrail1(j)] = extract_info(opt_initial_vec1(:,j),p,funs);
...
[opt_initial_vec2(:,1),fval,exitflag,output]= fminsearch(@(guess_init) cost_function_matlab(guess_init,p,funs),old_init_v2(:,1),options);

[TOF_v2(1),m_fuel_v2(1),dV_v2(1),flip_t_v2(1),e_s_v2(1),leadvtrail2(1)] = extract_info(opt_initial_vec2(:,1),p,funs);
...
[opt_initial_vec2(:,j),fval,exitflag,output]= fminsearch(@(guess_init) cost_function_matlab(guess_init,p,funs),old_init_v2(:,j),options);


[TOF_v2(j),m_fuel_v2(j),dV_v2(j),flip_t_v2(j),e_s_v2(j),leadvtrail2(j)] = extract_info(opt_initial_vec2(:,j),p,funs);
...
TOF_v = [TOF_v1 TOF_v2];
m_fuel_v = [m_fuel_v1 m_fuel_v2];
dV_v = [dV_v1 dV_v2];
flip_t_v = [flip_t_v1 flip_t_v2];
e_s_v = [e_s_v1 e_s_v2];
leadvtrail = [leadvtrail1 leadvtrail2];
thrust_v = [thrust_v1 thrust_v2];
opt_initial_vec = [opt_initial_vec1 opt_initial_vec2];
